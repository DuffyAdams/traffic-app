...
async def main():
    ...
    def is_duplicate(current_data, all_previous_data):
        for data in all_previous_data:
            if (data.get('Latitude') == current_data.get('Latitude') and
                data.get('Longitude') == current_data.get('Longitude') and
                data.get('Time') == current_data.get('Time')):
                return True
        return False
    ...
    while True:
        try:
            ...
            if has_data_changed(current_data, previous_data) and not is_duplicate(current_data, all_previous_data):
