...
from backend.api_support import BoundedTTLCache, RateLimiter, parse_rate_limit
...
class ApiSupportTests(unittest.TestCase):
    ...
    def test_ttl_cache_expires_and_evicts_least_recently_used_entry(self):
        ...
        with patch("backend.api_support.time.monotonic", return_value=10):
            cache.set("a", 1, ttl=5)
            cache.set("b", 2, ttl=5)
        ...
        with patch("backend.api_support.time.monotonic", return_value=16):
    ...
    def test_rate_limiter_enforces_window(self):
        ...
        with patch(
            "backend.api_support.time.monotonic",
            side_effect=[0, 0.1, 0.2, 61],
        ):
